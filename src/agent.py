...
def make_agent(
    ...
    *,
    ...
    # Set up tools and llm/tool nodes depending on mode.
    if mode == "browser":
        page = target
        TIMEOUT_MS = 2000
        ...
        async def llm_call(
            state: AgentState,
        ) -> Command[Literal["tool_node", "llm_call"]]:
                    ...
                    def walk(node: Any) -> None:
                        ...
                        if not node.get("hidden"):
                            if role or name:
                                ...
                                lines.append(
                                    f"- {role or '(no role)'}: {name or '(no name)'}{suffix}"
                                )
        ...
        async def tool_node(state: AgentState) -> Command[Literal["llm_call"]]:
            ...
            for tool_call in state["messages"][-1].tool_calls:
                ...
                if name == "finish":
                    ...
                    try:
                        reason = str(
                            tool_call.get("args", {}).get("reason", "")
                        ).strip().lower()
                    except Exception:
                        reason = ""

                    if reason.startswith("task success") or reason.startswith(
                        "success"
                    ):
                        status = "success"
        ...
        computer = target
        TIMEOUT_S = 2.0
        tools, tools_by_name, supported_tool_names = make_computer_tools(
            computer, TIMEOUT_S
        )
        model_with_tools = model.bind_tools(tools)
        ...
        async def llm_call(
                ...
                async def build_ui_manifest() -> str:
                    ...
                    def walk(node: Any) -> None:
                        if isinstance(node, dict):
                            ...
                            children = (
                                node.get("children") or node.get("AXChildren") or []
                            )
            ...
            current_context = getattr(computer, "name", None) or "computer://sandbox"
